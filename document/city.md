...
- All active **upcoming** events in each city (excludes completed events)
...
- Only returns events where `is_active = 1` **AND `event_date >= current date`** (upcoming events only)
